...
def get_loaders(rank, imgstr, resolution, timesteps, skip, batch_size=1, n_gpus=1, seed=42,  cond=False, use_train_set=False):
    ...
    """
    if imgstr == 'UCF101':
        train_dir = os.path.join(data_location, 'UCF-101/test/')
        #test_dir = os.path.join(data_location, 'UCF-101') # We use all 
        test_dir = os.path.join(data_location, 'UCF-101/test/')
        if cond:
            print("here")
        ...
        print(len(trainset))
        testset = VideoFolderDataset(test_dir, train=False, resolution=resolution, n_frames=timesteps, skip=skip, seed=seed)
        print(len(testset))
