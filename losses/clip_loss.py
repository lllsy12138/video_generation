...
from models.clip.clip_relevancy import ClipRelevancy
from tools.aug_utils import RandomSizeCrop
from tools.util import get_screen_template, get_text_criterion, get_augmentations_template
    ...
    def forward(self, outputs, inputs):
        ...
        if inputs["step"] < self.cfg["bootstrap_epoch"] and self.cfg["lambda_bootstrap"] > 0:
            ...
            if self.cfg["bootstrap_scheduler"] == "linear":
                lambda_bootstrap = self.cfg["lambda_bootstrap"] * (1 -
                                                                   (inputs["step"] + 1) / self.cfg["bootstrap_epoch"])
        ...
        if self.cfg["lambda_composition"] > 0:
            ...
            losses["loss_comp_dir"] = self.calculate_clip_dir_loss(all_inputs, all_outputs_composite,
                                                                   self.target_comp_e)
