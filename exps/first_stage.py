...
from models.autoencoder.autoencoder_vit_test import ViTAutoencoder
from losses.perceptual import LPIPSWithDiscriminator
from losses.clip_loss import LossG
...
def first_stage(rank, args):
    ...
    if args.n_gpus > 1:
        ...
        criterion = torch.nn.parallel.DistributedDataParallel(criterion,
                                                      ...
                                                      find_unused_parameters=False)
    fp = args.amp
    first_stage_train(rank, model, opt, d_opt, criterion, train_loader, test_loader, args.first_model, fp, args.n_gpus, logger)
