...
from models.clip.clip_extractor import ClipExtractor
...
class ViTAutoencoder(nn.Module):
    ...
    def encode(self, x):
        ...
        y = read_video('../../datasets/UCF-101/ApplyEyeMakeup/v_ApplyEyeMakeup_g12_c04.avi')[0]
        y = y[0:16].float().permute(3,0,1,2) # c t h w
        y = resize_crop(y, self.res)
        y = y.unsqueeze(0).cuda(0) # b t c h w

        clip_extractor = ClipExtractor()

        
        h = self.encoder(x)
        #h_modify = self.encoder(y)
        h_modify = clip_extractor(y)
        h = rearrange(h, 'b (t h w) c -> b c t h w', t=self.s, h=self.res//(2**self.down))
        h_modify = rearrange(h_modify, 'b (t h w) c -> b c t h w', t=self.s, h=self.res//(2**self.down))

        #修改后的h_xy
        #h_xy = rearrange(h_modify, 'b c t h w -> (b h w) t c')
        ...
        #把h_xy换成其他特征向量
        h_xy = self.pre_xy(h_xy) # b c h w
        h_yt = self.pre_yt(h_yt) # b c t w
        h_xt = self.pre_xt(h_xt) # b c t h
        ...
        h_xy = h_xy.unsqueeze(-3).expand(-1,-1,self.s,-1, -1) # (b, c, t, h, w)
        h_yt = h_yt.unsqueeze(-2).expand(-1,-1,-1,self.res//(2**self.down),-1) # (b, c, t, h, w)
        h_xt = h_xt.unsqueeze(-1).expand(-1,-1,-1,-1,self.res//(2**self.down)) # (b, c, t, h, w)
